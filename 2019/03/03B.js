    ...
    while(i < horizontalLines.length) {
        ...
        while (j < verticalLines.length) {
            ...
            if (isInBetween(horizontalLine[0], ...verticalLine[1]) && isInBetween(verticalLine[0],...horizontalLine[1])) {
                ...
                let totalPathDistance = horizontalLine[2] + verticalLine[2];
                let intersectionDistance = Math.abs(horizontalLine[1][0] - verticalLine[0]) + Math.abs(verticalLine[1][0] - horizontalLine[0]);

                result.push(totalPathDistance + intersectionDistance);
